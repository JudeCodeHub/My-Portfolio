...
interface CertFolder {
  folderName: string;
  isOpenByDefault: boolean;
  files: CertFile[];
}

const CERTIFICATIONS_DATA: CertFolder[] = [
  {
...
export const CertificationsSection = () => {
  ...
  const [selectedCert, setSelectedCert] = useState<CertFile | null>(null);
  ...
  const handlePrev = (e: MouseEvent) => {
    e.stopPropagation();
    if (hasPrev && activeFolderIdx !== null) {
      setSelectedCert(
  ...
  const handleNext = (e: MouseEvent) => {
    e.stopPropagation();
    if (hasNext && activeFolderIdx !== null) {
      setSelectedCert(
        CERTIFICATIONS_DATA[activeFolderIdx].files[activeFileIdx + 1],
      );
    }
  ...
  return (
    <section
      id="certifications"
      className="w-full md:min-h-screen lg:min-h-[85vh] py-12 md:py-10 lg:py-8 xl:py-24 relative overflow-hidden flex flex-col items-center max-w-[100vw] scroll-mt-10 lg:scroll-mt-0"
    >
      ...
      <div className="w-full max-w-5xl px-4 md:px-8 z-10">
        <div className="w-full min-h-[400px] md:h-112.5 xl:h-[650px] md:min-h-0 bg-white dark:bg-[#0a0a0a] rounded-xl border border-slate-200 dark:border-white/10 overflow-hidden shadow-2xl flex flex-col">
