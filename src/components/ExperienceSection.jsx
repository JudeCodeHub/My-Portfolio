...
import { EXPERIENCE_JOURNEY, EDUCATION_JOURNEY } from "@/data/journey";
...
// Per-column color tone. Only the two var *names* below are referenced by
// Tailwind classes (kept static so the build can see them); the actual
// orange/amber values come from --exp-accent/--edu-accent etc. in index.css
// and are applied locally via the --tone-* custom properties set inline on
// each card/rail, so every visual here reacts to the theme toggle for free.
const TONE = {
  exp: {
    label: "experience",
    Icon: Briefcase,
...
// Types out each bullet line-by-line like a terminal log being written live,
// once the card has scrolled into view. Falls back to showing everything
// immediately under prefers-reduced-motion.
function TypedBullets({ bullets, active, reduceMotion }) {
...
function JourneyCard({ entry, tone, index, reduceMotion }) {
  ...
  return (
    ...
    >
      ...
      >
        {/* macOS-terminal-style title bar */}
        ...
        <div className="relative flex flex-col flex-1 p-6">
          ...
          {/* Glowing pulse — echoes the "current" timeline dot, gives the
              card a deliberate finishing touch instead of stopping flat. */}
...
function JourneyColumn({ kind, entries, orderClass }) {
...
export const ExperienceSection = () => {
  return (
    ...
    >
      ...
      <div className="w-full max-w-5xl z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
        <JourneyColumn
          kind="edu"
          entries={EDUCATION_JOURNEY}
          orderClass="order-2 lg:order-1 lg:col-span-5"
        />
        <JourneyColumn
          kind="exp"
          entries={EXPERIENCE_JOURNEY}
          orderClass="order-1 lg:order-2 lg:col-span-7"
        />
      </div>
    </section>
  );
