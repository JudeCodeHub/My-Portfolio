...
function ContactInfoItem({
  ...
  label,
  value,
}: Pick<ContactInfoEntry, "icon" | "label" | "value">) {
  const inner = (
...
export const ContactSection = () => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (!serviceId || !templateId || !publicKey) {
      ...
      toast({
        title: "Contact form is temporarily unavailable",
        description:
    ...
    try {
      const quotaRes = await fetch("/api/contact", { method: "POST" });
      if (!quotaRes.ok) {
        const body = await quotaRes.json().catch(() => null);
        toast({
          title: "Message limit reached",
          description:
            body?.error ||
            "Please try again tomorrow, or reach out via social media instead.",
          variant: "destructive",
        });
      ...
      console.error("Contact quota check failed:", error);
      toast({
