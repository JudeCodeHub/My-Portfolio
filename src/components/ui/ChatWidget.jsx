...
export default function ChatWidget() {
  ...
  useEffect(() => {
    ...
    if (!el) return;
    // Only auto-scroll if the user is already near the bottom, so scrolling
    // up to read earlier messages isn't yanked back down on every streamed
    // token (this was previously firing on every chunk during streaming).
  ...
  async function sendMessage() {
    ...
    const nextMessages = [...messages, { role: "user", content: text }].slice(
