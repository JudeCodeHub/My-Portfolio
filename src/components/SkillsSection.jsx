...
const graphData = [
  ...
  },
  {
    id: "devops",
    title: "DEVOPS",
    skills: ["git", "docker", "github2", "kubernetes", "linux", "aws", "terraform"]
  }
];

export const SkillsSection = () => {
  return (
    <section id="skills" className="w-full min-h-screen pt-20 pb-8 flex flex-col justify-start items-center px-4 relative overflow-hidden scroll-mt-20">
      ...
      <div className="w-full max-w-5xl flex flex-col gap-6 md:gap-8 z-10 mx-auto">
        {graphData.map((node, i) => (
          <div key={node.id} className="relative flex flex-col md:flex-row items-center gap-4 md:gap-8 w-full">
            ...
            >
               ...
               <div className="absolute inset-2 rounded-full border border-orange-500/20 animate-[spin_10s_linear_infinite]" style={{ borderTopColor: 'transparent' }} />
               <div className="absolute inset-4 rounded-full border border-orange-500/20 animate-[spin_15s_linear_infinite_reverse]" style={{ borderBottomColor: 'transparent' }} />
            ...
            {/* Skill Nodes Cluster */}
            <div className="flex-1 flex flex-wrap justify-center md:justify-start items-center gap-3 md:gap-5 relative z-10">
              {node.skills.map((skill, sIdx) => (
                ...
                >
                  ...
                  <div className="absolute -bottom-6 font-mono font-bold text-[9px] md:text-[10px] text-white/90 group-hover:text-orange-500 tracking-widest uppercase transition-colors duration-300 pointer-events-none whitespace-nowrap">
                    {skill === 'github2' ? 'github' : skill === 'nextjs2' ? 'nextjs' : skill}
                  </div>
                </motion.div>
              ))}
            </div>
